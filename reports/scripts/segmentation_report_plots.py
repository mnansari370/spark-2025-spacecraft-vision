...
def safe_load_mask(p: Path) -> np.ndarray | None:
    """Load a single-channel label mask as uint8 (H,W). Return None if bad."""
    try:
        if p.stat().st_size == 0:
            return None
...
def main():
    ...
    ap.add_argument("--mask_dir", default="inference_results/segmentation/predicted_masks",
                    help="Folder containing test_XXXXX_layer.png")
    ap.add_argument("--limit", type=int, default=0, help="If >0, use only first N masks")
    ap.add_argument("--sample_vis", type=int, default=20, help="How many masks to sample for visualization")
    ...
    print("Using first masks (limit):", len(pngs))

    # If no masks, don't crash: just explain what to do.
    if len(pngs) == 0:
        msg = (
            "No masks found.\n"
            f"Expected PNGs in: {mask_dir}\n"
            "Run: python -u scripts/segmentation/run_segmentation_inference.py --limit 200 --no_tta\n"
        )
        raise RuntimeError(msg)

    # Accumulators
    total_counts = np.zeros(3, dtype=np.int64)  # bg, body, panels
    per_image_ratios = []  # list of [body_ratio, panels_ratio]
    good_files = []
    ...
    per_image_ratios = np.array(per_image_ratios, dtype=np.float32)  # shape (N,2) if N>0

    if len(good_files) == 0:
        raise RuntimeError("All masks were unreadable or empty. Check your inference output.")
    ...
    # ---- Plot 2: per-image ratios histograms (body & panels)
    # Only do this if we have enough samples
    if per_image_ratios.ndim == 2 and per_image_ratios.shape[0] > 0:
        plt.figure()
    ...
    # ---- Optional visualization: save some example masks as images (good for report)
    rng = np.random.default_rng(args.seed)
    k = min(args.sample_vis, len(good_files))
    picks = rng.choice(len(good_files), size=k, replace=False)

    # Save a small grid of masks
    cols = 5
