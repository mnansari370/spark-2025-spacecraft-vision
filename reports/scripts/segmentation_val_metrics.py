...
def iter_mask_files(root: Path):
    """
    Recursively collect mask files with extensions png/jpg/jpeg
    """
    exts = {".png", ".jpg", ".jpeg"}
    for p in root.rglob("*"):
...
def load_mask(path: Path) -> np.ndarray:
    ...
    If image is RGB accidentally, take channel 0.
    """
...
def plot_confusion(conf: np.ndarray, class_names, out_path: Path):
    ...
    # write values (small only if matrix is small; it is)
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=0, help="0 means all, else only first N matched pairs")
